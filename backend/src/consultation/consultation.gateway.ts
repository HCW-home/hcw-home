  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket): Promise<void> {
    ...
    if (this.server) {
      this.server.to(`consultation:${cId}`).emit('system-message', {
        type: 'JOINED',
        userId: uId,
  ...
  async handleReadReceipt(
    @MessageBody()
    data: { consultationId: number; userId: number; messageId: number },
  ) {
    await this.databaseService.markMessageAsRead(
      data.messageId,
      data.userId,
    );
    this.server
